...
`;
    ...
    var EventHandler = /** @class */ (function () {
        ...
        EventHandler.prototype.handle = function (e) {
            var data = e.data;
            ...
            switch (topic) {
                ...
                case 'coastline':
                    ...
                    computeCoastline(this.mesher).then(function (c) {
                        ctx.postMessage({
                            'topic': 'coastline',
                ...
                case 'tracks':
                    ...
                    var m_1 = this.mesher;
                    var h_1 = function () {
                        computeTracks(m_1).then(function (tracks) {
                            ctx.postMessage({
                                'topic': 'tracks',
                        ...
                        }).finally(function () { return setTimeout(h_1, 10000); });
                    };
                    setTimeout(h_1, 100);
                    break;
            }
    ...
    var trackPaint = Paint.complete(new Stroke(Colour.DIMGRAY, 2), Colour.LIGHTGREY);
    var trackOffsets = [new Offset(-5, -5), new Offset(-5, 5), new Offset(5, 5), new Offset(5, -5)];
    var trackZIndex = 1;
    ctx.onmessage = eh.handle;
    function computeCoastline(mesher) {
        return __awaiter(this, void 0, void 0, function () {
            var response, data, nbFeatures, meshes, i, feature, geometry, polylines, nbPolylines, j;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('https://ofmooseandmen.github.io/twinf/assets/ne_50m_coastline.json')];
                    case 1:
                        response = _a.sent();
                        return [4 /*yield*/, response.json()];
                    case 2:
                        data = _a.sent();
                        nbFeatures = data.features.length;
                        meshes = new Array();
                        for (i = 0; i < nbFeatures; i++) {
                            ...
                            if (geometry.type === "LineString") {
                                ...
                                for (j = 0; j < nbPolylines; j++) {
                                    meshes = meshes.concat(meshesOf(polylines[j], mesher));
                                }
                            }
                        }
                        return [2 /*return*/, new RenderableGraphic('coastlines', -1, meshes)];
                }
            });
        });
    }
    function meshesOf(coordinates, mesher) {
        var nbCoordinates = coordinates.length;
        var positions = new Array();
        for (var k = 0; k < nbCoordinates; k++) {
            var coord = coordinates[k];
            /* longitude first, then latitude. */
            var point = LatLong.ofDegrees(coord[1], coord[0]);
            positions.push(point);
        }
        var shape = new GeoPolyline(positions, new Stroke(Colour.LIGHTSLATEGRAY, 1));
        return mesher.meshShape(shape);
    }
    function computeTracks(mesher) {
        return __awaiter(this, void 0, void 0, function () {
            var states, len, res, i, state, m;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetchStateVectors()];
                    case 1:
                        states = _a.sent();
                        len = states.length;
                        res = new Array();
                        for (i = 0; i < len; i++) {
                            ...
                            if (state.position !== undefined) {
                                m = mesher.meshShape(new GeoRelativePolygon(state.position, trackOffsets, trackPaint));
    ...
    function fetchStateVectors() {
        return __awaiter(this, void 0, void 0, function () {
            var response, data, prop, states;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('https://opensky-network.org/api/states/all')];
                    case 1:
                        response = _a.sent();
                        return [4 /*yield*/, response.json()];
                    case 2:
                        ...
                        for (prop in data) {
                            if (prop === 'states') {
                                states = data[prop];
                                return [2 /*return*/, states.map(StateVector.parse)];
                            }
                        }
                        return [2 /*return*/, []];
                }
    ...
    var StateVector = /** @class */ (function () {
        ...
        StateVector.parse = function (arr) {
            var res = new StateVector();
            res.icao24 = arr[0];
            if (arr[1] !== null) {
                res.callsign = arr[1];
            }
