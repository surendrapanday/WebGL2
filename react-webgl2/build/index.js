...
`;
    ...
    var DemoApp = /** @class */ (function () {
        function DemoApp(gl) {
            var _this = this;
            var linkoping = LatLong.ofDegrees(58.4108, 15.6214);
            var def = new WorldDefinition(linkoping, Length.ofKilometres(8000), Angle.ofDegrees(0), Colour.ALICEBLUE);
            this.world = new World(gl, def);
            this.events = new Events();
            this.animator = new Animator(function () { return _this.world.render(); }, this.events, 60);
            ...
            this.worker.onmessage = function (e) {
                ...
                switch (topic) {
                    case 'coastline':
                        var c = RenderableGraphic.fromLiteral(JSON.parse(e.data.payload));
                        _this.world.insert(c);
                        _this.worker.postMessage({
                            'topic': 'tracks'
                        });
                        break;
                    case 'tracks':
                        var tracks = JSON.parse(e.data.payload).map(RenderableGraphic.fromLiteral);
                        for (var i = 0; i < tracks.length; i++) {
                            _this.world.insert(tracks[i]);
                        }
                        _this.events.fireEvent('tracksChanged', tracks.length);
                        break;
                    case 'error':
        ...
        DemoApp.prototype.addEventListener = function (name, handler) {
        ...
        DemoApp.prototype.handleKeyboardEvent = function (evt) {
            var delta = DemoApp.DELTA.get(evt.key);
            var factor = DemoApp.FACTOR.get(evt.key);
            if (delta !== undefined) {
        ...
        DemoApp.prototype.fireCentreChanged = function () {
            var c = this.world.centre();
            var lat = c.latitude().degrees();
            var lon = c.longitude().degrees();
            var ll = Math.abs(lat).toFixed(4)
                + (lat < 0 ? 'S' : 'N')
                + ' ' + Math.abs(lon).toFixed(4)
                + (lon < 0 ? 'W' : 'E');
            this.events.fireEvent('centreChanged', ll);
        };
        DemoApp.prototype.fireRangeChanged = function () {
            var r = this.world.range().kilometres().toFixed(0) + ' km';
    ...
    var Events = /** @class */ (function () {
        ...
        Events.prototype.addEventListener = function (name, handler) {
    ...
    var FpsTracker = /** @class */ (function () {
        function FpsTracker(events) {
            this.start = performance.now();
            this.frames = 0;
            this.events = events;
        }
        FpsTracker.prototype.doneRendering = function () {
            this.frames = this.frames + 1;
            var now = performance.now();
            var delta = now - this.start;
            if (delta >= 1000) {
                var fps = Math.round(this.frames / delta * 1000);
    ...
    var Animator = /** @class */ (function () {
        function Animator(callback, events, fps) {
        ...
        Animator.prototype.render = function () {
            var _this = this;
            this.handle = requestAnimationFrame(function () { return _this.render(); });
            this.now = performance.now();
